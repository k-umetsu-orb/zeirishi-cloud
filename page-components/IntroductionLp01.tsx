/**
 * Introduction LP (weekend ads, no phone CTA) - /introduction-lp01
 * Copied from Introduction.tsx (/introduction). Design: zeiri4.com/shoukai/ inspired.
 */
import { useState } from "react";
import { usePageTitle } from "@/lib/usePageTitle";
import Image from "next/image";
import { useRouter } from "next/router";
import { ArrowRight, Building2, CalendarCheck, ChevronDown, CircleCheck, FilePenLine, Landmark, MessageCircle, Cloud, FileText, MapPin, Search, Calculator, Briefcase, Handshake, ClipboardList, ClipboardCheck, JapaneseYen, Mail, UserRound, Users } from "lucide-react";
...
function ConsultationStartForm({ variant = "section" }: { variant?: "hero" | "section" }) {
  ...
  function proceed() {
    if (!selected) return;
    void router.push({ pathname: "/introduction-lp01/step", query: { clientType: selected } });
  }
...
function IntroductionStickyCta() {
  return (
    <aside className="intro-sticky-cta" aria-label="無料相談">
...
function IntroductionForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push("/introduction-lp01/thanks");
    } catch {
...
export default function IntroductionLp01({ content }: { content?: IntroductionPageContent }) {
  usePageTitle(
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <style jsx global>{`
        ...
        /* LP01: 電話CTAが無いため、残るフォームCTAを中央寄せ・約1.5倍に拡大する */
        .intro-concerns__cta:has(.testlp-form-cta:only-child) {
          display: flex;
          justify-content: center;
          align-items: center;
        }

        .intro-concerns__cta:has(.testlp-form-cta:only-child):not(.intro-sticky-cta__inner) {
          max-width: none;
          border: 0;
          background: transparent;
          box-shadow: none;
        ...
        .intro-concerns__cta:has(.testlp-form-cta:only-child):not(.intro-sticky-cta__inner) .testlp-form-cta {
          width: min(100%, 930px);
          min-height: 108px;
          border: 0;
          border-radius: 20px;
        ...
        .intro-concerns__cta:has(.testlp-form-cta:only-child):not(.intro-sticky-cta__inner) .testlp-form-cta__main {
          display: flex;
          align-items: center;
          justify-content: center;
          gap: 14px;
        ...
        .intro-concerns__cta:has(.testlp-form-cta:only-child):not(.intro-sticky-cta__inner) .testlp-form-cta__free {
          width: 62px;
          height: 62px;
          flex: 0 0 auto;
          border-width: 0;
        ...
        .intro-concerns__cta:has(.testlp-form-cta:only-child):not(.intro-sticky-cta__inner) .testlp-form-cta__free svg {
          width: 33px;
          height: 33px;
        }

        .intro-concerns__cta:has(.testlp-form-cta:only-child):not(.intro-sticky-cta__inner) .testlp-form-cta__body {
          flex: 0 1 auto;
          min-width: 0;
        }

        .intro-concerns__cta:has(.testlp-form-cta:only-child):not(.intro-sticky-cta__inner) .testlp-form-cta__label {
          display: block;
          margin-bottom: 3px;
          font-size: 0.9rem;
        }

        .intro-concerns__cta:has(.testlp-form-cta:only-child):not(.intro-sticky-cta__inner) .testlp-form-cta__title {
          font-size: clamp(1.15rem, 2vw, 1.55rem);
        }

        .intro-concerns__cta:has(.testlp-form-cta:only-child):not(.intro-sticky-cta__inner) .testlp-form-cta__note {
          margin-top: 4px;
          font-size: 0.84rem;
        }

        .intro-concerns__cta:has(.testlp-form-cta:only-child):not(.intro-sticky-cta__inner) .testlp-form-cta__arrow {
          position: static;
          width: 42px;
          height: 42px;
          flex: 0 0 auto;
        ...
        .intro-concerns__cta:has(.testlp-form-cta:only-child):not(.intro-sticky-cta__inner) .testlp-form-cta__arrow-svg {
          width: 22px;
          height: 22px;
        }

        .intro-sticky-cta__inner:has(.testlp-form-cta:only-child) .testlp-form-cta {
          border-right: 0;
          border-bottom: 0;
        }

        @media (min-width: 768px) {
          .intro-sticky-cta__inner:has(.testlp-form-cta:only-child) {
            width: calc(min(100% - 64px, 1280px) * 0.75);
          }
        }
        ...
        @media (max-width: 767px) {
          ...
          .intro-sticky-cta__inner:has(.testlp-form-cta:only-child) .testlp-form-cta__label {
            font-size: 0.75rem;
            white-space: nowrap;
          }

          .intro-sticky-cta__inner:has(.testlp-form-cta:only-child) .testlp-form-cta__title {
            font-size: 1.3rem;
          }

          .intro-sticky-cta__inner:has(.testlp-form-cta:only-child) .testlp-form-cta__note {
            font-size: 0.68rem;
          }
