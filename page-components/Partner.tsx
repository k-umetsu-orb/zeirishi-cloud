...
const serviceOptions = ["税理士ドットコム", "税理士紹介センタービスカス", "税理士コンシェルジュ", "税理士紹介エージェント", "その他"];
const concernOptions = ["他社サービスを利用しているが費用が高い", "新規の問い合わせが来ない", "営業の受注率が低い", "受注単価が上がらない", "その他"];
...
export default function Partner() {
  ...
  const [form, setForm] = useState({ officeName: "", employeeCount: "", taxAccountantCount: "", name: "", email: "", phone: "", message: "", services: [] as string[], servicesOther: "", concerns: [] as string[], concernsOther: "" });
        ...
        <section id="contact" className="partner-contact" aria-labelledby="contact-heading">
          <div className="partner-shell partner-contact__inner">
            ...
            <form className="partner-contact__form" onSubmit={submit}>
              ...
              <label>従業員数<span className="partner-form__required">必須</span><input required value={form.employeeCount} onChange={(event) => setForm({ ...form, employeeCount: event.target.value })} placeholder="例）10名" /></label>
              <label>所属税理士数<span className="partner-form__required">必須</span><input required value={form.taxAccountantCount} onChange={(event) => setForm({ ...form, taxAccountantCount: event.target.value })} placeholder="例）3名" /></label>
              <label>お名前<span className="partner-form__required">必須</span><input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="例）山田 太郎" /></label>
              ...
              <fieldset className="partner-contact__checkbox-group">
                <legend>ご利用中のサービス<span className="partner-form__optional">任意</span></legend>
                <div className="partner-contact__checkboxes">
                  {serviceOptions.flatMap((option, index) => [
                    ...(index === serviceOptions.length - 1 ? [<div key="service-break" className="partner-contact__checkbox-break" aria-hidden="true" />] : []),
                    <label key={option} className="partner-contact__checkbox">
                      <input
                        type="checkbox"
                        checked={form.services.includes(option)}
                        onChange={() => setForm({ ...form, services: form.services.includes(option) ? form.services.filter((v) => v !== option) : [...form.services, option] })}
                      />
                      {option}
                    </label>,
                  ])}
                ...
                {form.services.includes("その他") && (
                  <input className="partner-contact__checkbox-other" value={form.servicesOther} onChange={(event) => setForm({ ...form, servicesOther: event.target.value })} placeholder="その他の内容をご記入ください" />
                )}
              </fieldset>
              <fieldset className="partner-contact__checkbox-group">
                <legend>現在のお悩みごと<span className="partner-form__optional">任意</span></legend>
                <div className="partner-contact__checkboxes">
                  {concernOptions.flatMap((option, index) => [
                    ...(index === concernOptions.length - 1 ? [<div key="concern-break" className="partner-contact__checkbox-break" aria-hidden="true" />] : []),
                    <label key={option} className="partner-contact__checkbox">
                      <input
                        type="checkbox"
                        checked={form.concerns.includes(option)}
                        onChange={() => setForm({ ...form, concerns: form.concerns.includes(option) ? form.concerns.filter((v) => v !== option) : [...form.concerns, option] })}
                      />
                      {option}
                    </label>,
                  ])}
                ...
                {form.concerns.includes("その他") && (
                  <input className="partner-contact__checkbox-other" value={form.concernsOther} onChange={(event) => setForm({ ...form, concernsOther: event.target.value })} placeholder="その他の内容をご記入ください" />
                )}
              </fieldset>
              <label className="partner-contact__message">お問い合わせ内容<span className="partner-form__optional">任意</span><textarea value={form.message} onChange={(event) => setForm({ ...form, message: event.target.value })} placeholder={"・費用について詳しく知りたい\n・毎月の紹介件数を教えてほしい\n・掲載までの流れを知りたい"} rows={4} /></label>
